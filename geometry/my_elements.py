...
tbl = pd.read_csv('./elements.csv')
tbl.atomicMass = tbl.atomicMass.map(lambda x: float(x.replace('(', '').replace(')', '').replace('[', '').replace(']', '')))
tbl.symbol = tbl.symbol.map(lambda x: str(x.replace(' ', '')))

# rel_mass = dict(H=1.0079, He=4.00, Li=6.94, Be=9.01, B=10.81, C=12.01,
...
# symbol = dict(zip(range(1,len(symbol_list)), symbol_list))
